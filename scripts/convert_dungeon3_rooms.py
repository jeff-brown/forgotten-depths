#!/usr/bin/env python3
"""Convert dungeon3_rooms.json to Forgotten Depths room format."""
...
def clean_title(title):
    ...
    title = re.sub(r'^You\'re in a\s+', '', title, flags=re.IGNORECASE)
    title = re.sub(r'^You\'re in an\s+', '', title, flags=re.IGNORECASE)
    title = re.sub(r'^You\'re in the\s+', '', title, flags=re.IGNORECASE)
    title = re.sub(r'^You\'re in\s+', '', title, flags=re.IGNORECASE)
    title = re.sub(r'^You\'re at\s+', '', title, flags=re.IGNORECASE)
    title = title.rstrip('.')
    if title:
...
def convert_room(room_data, room_id_map):
    ...
    room_id = f"dungeon3_{room_data['id']}"
    ...
    if not title_text or title_text.strip() in ['', '0', str(room_data['id'])]:
        title = f"Dungeon Level 3 - Room {room_data['id']}"
    else:
        # Clean and capitalize
        title = clean_title(title_text.strip())
        if len(title) > 60:
    ...
    for exit_data in room_data.get('exits', []):
        ...
        # Map room ID
        target_room_id = f"dungeon3_{to_room}"
    ...
    if lairs:
        for lair in lairs:
            ...
            is_lair = True
            lair_monster = f"dungeon3_mob_{mob_id}"
            break
    ...
    room = {
        ...
        'area_id': 'dungeon3',
        'is_safe': False,
        'light_level': 0.05,  # Very dark - deepest level
        'exits': exits
    }
...
def main():
    ...
    # Paths
    input_file = Path('data/world/areas/dungeon3_rooms.json')
    output_dir = Path('data/world/rooms/dungeon3')
    ...
    for room in converted_rooms:
        for direction, target_id in room.get('exits', {}).items():
            if target_id not in all_room_ids and not target_id.startswith('dungeon3_-'):
    ...
    # Create area file
    area_file = Path('data/world/areas/dungeon3.json')
    area_data = {
        'id': 'dungeon3',
        'name': 'Dungeon Level 3',
        'description': 'The deepest and most treacherous level of the dungeon, shrouded in near-total darkness. Only the bravest adventurers dare venture here.',
        'level_range': [10, 20],
