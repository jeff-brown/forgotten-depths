#!/usr/bin/env python3
"""Convert dungeon2_rooms.json to Forgotten Depths room format."""
...
def convert_room(room_data, room_id_map):
    ...
    room_id = f"dungeon2_{room_data['id']}"
    ...
    if not title_text or title_text.strip() in ['', '0', str(room_data['id'])]:
        title = f"Dungeon Level 2 - Room {room_data['id']}"
    else:
    ...
    for exit_data in room_data.get('exits', []):
        ...
        # Map room ID
        target_room_id = f"dungeon2_{to_room}"
    ...
    if lairs:
        for lair in lairs:
            ...
            lair_monster = f"dungeon2_mob_{mob_id}"
            break
    ...
    room = {
        ...
        'area_id': 'dungeon2',
        'is_safe': False,
        'light_level': 0.1,  # Darker than dungeon1
        'exits': exits
    }
...
def main():
    ...
    # Paths
    input_file = Path('data/world/areas/dungeon2_rooms.json')
    output_dir = Path('data/world/rooms/dungeon2')
    ...
    for room in converted_rooms:
        for direction, target_id in room.get('exits', {}).items():
            if target_id not in all_room_ids and not target_id.startswith('dungeon2_-'):
    ...
    # Create area file
    area_file = Path('data/world/areas/dungeon2.json')
    area_data = {
        'id': 'dungeon2',
        'name': 'Dungeon Level 2',
        'description': 'A deeper, darker dungeon level with more dangerous creatures and greater treasures.',
        'level_range': [5, 15],
