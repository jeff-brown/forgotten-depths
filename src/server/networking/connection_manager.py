"""Manages client connections and communication using the integrated telnet server."""
...
class Connection:
    ...
    def __init__(self, player_id: int, telnet_server: TelnetServer):
    ...
    def send(self, message: str):
        ...
        if self.connected:
            self.telnet_server.send_message(self.player_id, message)

    def disconnect(self):
        """Disconnect the client."""
        if self.connected:
            self.telnet_server.disconnect_player(self.player_id)
...
class ConnectionManager:
    """Manages all client connections using the integrated telnet server."""
    ...
    def initialize(self, host: str = "localhost", port: int = 4000, event_system: Optional[EventSystem] = None):
        """Initialize the telnet server."""
        self.telnet_server = TelnetServer(host, port)
        ...
        self.logger.info(f"Connection manager initialized for {host}:{port}")

    def start_server(self, host: str = "localhost", port: int = 4000):
        ...
        server_thread = threading.Thread(target=self.telnet_server.start, daemon=True)
        server_thread.start()
    ...
    def _handle_player_connect(self, player_id: int):
        ...
        connection = Connection(player_id, self.telnet_server)
        self.connections[player_id] = connection

        self.logger.info(f"New connection established: player {player_id}")
    ...
    def _handle_player_disconnect(self, player_id: int):
        ...
        if player_id in self.connections:
            ...
            self.logger.info(f"Connection closed: player {player_id}")
    ...
    def broadcast(self, message: str, exclude_player: Optional[int] = None):
        """Send a message to all connected clients."""
        if self.telnet_server:
            self.telnet_server.broadcast_message(message, exclude_player)

    def broadcast_to_room(self, message: str, room_id: str, exclude_player: Optional[int] = None):
        """Send a message to all players in a specific room."""
        if self.telnet_server:
            self.telnet_server.send_message_to_room(message, room_id, exclude_player)
